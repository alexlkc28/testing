...
class RmaProcess(models.Model):
    ...
    rma_line_ids = fields.One2many('rma.order.line', 'rma_process_id',
                                   string='Lines')
    ...
    def button_validate(self):
        ...
        for invoice in self.sale_order_id.invoice_ids:
            ...
            action['context'] = (
                {'rma_mov_ids': self.sale_order_id.invoice_ids.ids})
...
class SaleOrder(models.Model):
    ...
    def rma_button(self):
        """rma button"""
        if self.order_line.filtered(lambda line: not line.rma_line_id):
    ...
    def get_rma(self):
        ...
        return {
            ...
            'domain': [('id', 'in', self.env['rma.process'].search(
